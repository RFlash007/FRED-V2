...
def init_stm_db():
    ...
    try:
        with sqlite3.connect(STM_DB_PATH) as conn:
            ...
            """)
        olliePrint_simple("STM database initialized successfully")
    except Exception as e:
        olliePrint_simple(f"Failed to initialize STM database: {e}", level='error')
        raise
...
Key learnings from ONLY the marked ">>>" messages:"""
        ...
        olliePrint_simple(f"Failed to analyze conversation segment: {e}", level='error')
        return None
    ...
    try:
        ...
        if existing_embeddings:
            ...
            if max_similarity > config.STM_SIMILARITY_THRESHOLD:
                olliePrint_simple(f"STM content too similar (similarity: {max_similarity:.3f}), skipping storage")
                return False
        ...
        olliePrint_simple(f"Stored new STM: {content[:100]}...")
        return True
        
    except Exception as e:
        olliePrint_simple(f"Failed to check similarity and store: {e}", level='error')
        return False
...
(END RECENTLY POSSIBLY RELATED CONTEXT)"""
        ...
        olliePrint_simple(f"Failed to query STM context: {e}", level='error')
        return ""
...
def process_stm_analysis(conversation_history: List[Dict], last_analyzed_index: int, current_total: int):
    ...
    try:
        ...
        olliePrint_simple(f"Analyzing STM for {message_range}")
        ...
        if learnings:
            ...
            if success:
                olliePrint_simple(f"STM analysis complete for {message_range}")
            else:
                olliePrint_simple(f"STM analysis skipped (duplicate) for {message_range}")
        else:
            olliePrint_simple(f"STM analysis failed for {message_range}", level='warning')
            
    except Exception as e:
        olliePrint_simple(f"STM background processing failed: {e}", level='error')
