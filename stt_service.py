...
class STTService:
    ...
    def initialize(self):
        ...
        try:
            ...
            olliePrint_simple(f"Speech recognition matrix: {cpu_cores} cores, {available_memory:.1f}GB RAM")
            olliePrint_simple(f"Initializing Whisper large-v3 (quantized) on {device.upper()}")
            ...
            olliePrint_simple(f"Loading {model_size} model with {compute_type} quantization")
            ...
            try:
                ...
                olliePrint_simple("High priority mode enabled for real-time processing")
            except Exception as e:
                olliePrint_simple(f"Could not set process priority: {e}")
            
            self.is_initialized = True
            olliePrint_simple(f"Speech recognition online - {model_size} quantized model ready")
            return True
            
        except Exception as e:
            olliePrint_simple(f"Speech recognition initialization failed: {e}")
            return False
    ...
    def start_processing(self, callback):
        ...
        olliePrint_simple("STT processing started - waiting for wake word...")
        return True
            ...
            try:
                ...
                if isinstance(queue_item, tuple):
                    ...
                    if from_pi and not self._pi_threshold_set:
                        ...
                        olliePrint_simple(f"[DEBUG] Pi silence threshold set to {self.silence_threshold:.6f}")
                    ...
                    if from_pi:
                        ...
                        if self._pi_chunk_counter % 20 == 0:
                            olliePrint_simple(f"[DEBUG] Processing Pi audio chunk #{self._pi_chunk_counter} ({len(audio_chunk)} samples)")
                else:
                    audio_chunk, from_pi = queue_item, False
                    olliePrint_simple(f"[DEBUG] Processing local audio chunk ({len(audio_chunk)} samples)")
                ...
                if self.is_listening:
                    ...
                    if now - self._last_level_log > 0.5:  # print at most twice per second
                        olliePrint_simple(f"\rAudio level: {audio_level:.6f} (Threshold: {self.silence_threshold:.6f})", end="")
                ...
                if audio_level > self.silence_threshold:
                    ...
                    if self._debug_counter % 5 == 0:  # Every 5th detection
                        olliePrint_simple(f"[PROCESSING] Audio level: {audio_level:.6f}")
                    ...
                    try:
                        ...
                        if from_pi and segments and self._debug_counter % 10 == 0:  # Concise Pi logging
                            olliePrint_simple(f"[ARMLINK] Processed {len(segments)} speech segments")
                        ...
                        for segment in segments:
                            ...
                            if any(phrase in text for phrase in self._ignore_phrases):
                                if self._debug_counter % 20 == 0:  # Occasional hallucination logging
                                    olliePrint_simple(f"[FILTER] Blocked hallucination: '{text[:30]}...'")
                                continue
                            ...
                            if text and len(text.split()) > 0:
                                ...
                                olliePrint_simple(f"[RECOGNITION] {source_type}: {text}")
                                ...
                                if not self.is_listening:
                                    ...
                                    if wake_word_found:
                                        olliePrint_simple(f"[WAKE] F.R.E.D. activated - listening mode engaged")
                                ...
                                if self.is_listening:
                                    ...
                                    if stop_word_found:
                                        olliePrint_simple(f"[SLEEP] F.R.E.D. deactivated")
                                    ...
                                    if len(words) > 1:  # More than one word
                                        ...
                                        olliePrint_simple(f"[BUFFER] Added: '{text}' ({len(self.speech_buffer)} segments)")
                                        print_transcription_to_terminal(f"[COMMAND] {text} (Buffer: {len(self.speech_buffer)})", "VOICE COMMAND")

                    except Exception as e:
                        olliePrint_simple(f"[ERROR] Transcription failed: {str(e)}")
                        olliePrint_simple(f"Error during transcription: {str(e)}")
                else:
                    ...
                    if (self.is_listening and self.speech_buffer and 
                        ...
                        try:
                            ...
                            olliePrint_simple(f"Error in callback processing: {str(e)}")
                            self.is_listening = True

            except Exception as e:
                olliePrint_simple(f"Error in audio processing loop: {e}")
                olliePrint_simple(f"[DEBUG] Audio processing loop error: {e}")
                time.sleep(0.5)
    ...
    def _transcribe_audio(self, audio_chunk):
        ...
        try:
            ...
            olliePrint_simple(f"Error transcribing audio: {e}")
            return ""
    ...
    def transcribe_file(self, audio_file_path):
        ...
        try:
            ...
            olliePrint_simple(f"Error transcribing file: {e}")
            return ""
